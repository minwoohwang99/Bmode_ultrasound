...
int main()
{
    ...
    BmodeClass *currentScanline = scanlineHead; // declare BmodeClass pointer and assign scanlineHead to it
    dataBuffer *iterateData = dataHead;         // declare dataBuffer pointer and assign dataHead to it

    for (int i = 1; i < parameters->getNumScanline(); i++) // iterate numScanline times
    {
        iterateData = iterateData->next;                                    // advance iterateData to next entry
        currentScanline->next = new BmodeClass(parameters, iterateData, i); // create new Bmodeclass object using the parametrized constructor and assign the pointer next of Scanline to it
        currentScanline = currentScanline->next;                            // advance currentScanline to the next entry
        currentScanline->next = NULL;                                       // set next entry of currentScanline to NULL
    }
